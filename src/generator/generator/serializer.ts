...
export class TypeScriptSerializer implements Serializer {
  ...
  readonly singularize: Singularizer | undefined;
  ...
  serializeFile(
    ...
    let data = '';
    const singularize = this.singularize;
    ...
    data += this.serializeStatements(
      transform({
        ...
        tableNameNormalizer: singularize
          ? (name) => normalizeTableIdentifier(name, singularize)
          : undefined,
      }),
    );
  ...
  serializeIdentifier(node: IdentifierNode) {
    ...
    return normalizeTableIdentifier(node.name, this.singularize);
  }
